...
var arrayComuni = [];
$.getJSON("comuni.json",function(resp){
	for (c=0; c < resp.comuni.length; c++){
		arrayComuni.push(resp.comuni[c].nome);
		getLista(resp.comuni[c].istat,resp.comuni[c].nome);
	}
});
...
$.getJSON("proxy.php?format=0&url=www.portaleacque.salute.gov.it/PortaleAcquePubblico/rest/layer/PP/066103",function(pp){
	ppJSON = pp;
	...
	$.getJSON("proxy.php?format=0&url=www.portaleacque.salute.gov.it/PortaleAcquePubblico/rest/layer/AB/066103",function(ab){
		abJSON = ab;
		// Ordina l'array in base a CODICE crescente
		...
		zone = L.geoJson(abJSON,{
			filter: function(feature,layer){
				// Il filtro impedisce il rendering sulla mappa delle zone interne (Lago di San Domenico)
				if (feature.properties.CODICE != "5675" && feature.properties.CODICE != "5676" && feature.properties.CODICE != "5677" && feature.properties.CODICE != "5678" && feature.properties.CODICE != "5679"){
					return feature;
				}
			},
...
function getDetails(prop,geom){
	// Popup per aree non analizzate (classe 0) e per aree interne (N.B. controllo reso inutile dal filtro sul layer)
	if (prop.PP_CLASSE == 0 || prop.CODICE == "5675" || prop.CODICE == "5676" || prop.CODICE == "5677" || prop.CODICE == "5678" || prop.CODICE == "5679") {
		bootbox.dialog({
